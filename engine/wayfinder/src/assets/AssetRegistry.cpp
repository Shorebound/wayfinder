...
namespace Wayfinder
{
    ...
    bool AssetRegistry::BuildFromDirectory(const std::filesystem::path& rootDirectory, std::string& error)
    {
        ...
        if (!std::filesystem::exists(rootDirectory)) { return true; }
        ...
        for (const auto& entry : std::filesystem::recursive_directory_iterator(rootDirectory))
        {
            ...
            try
            {
                ...
                if (!document.contains(kAssetIdKey) || !document[kAssetIdKey].is_string()) { continue; }
                ...
                if (!AssetSchemaRegistry::ValidateDocument(assetTypeText, document, entry.path(), error)) { return false; }
                ...
                {
                    error =
                        "Duplicate asset_id '" + assetId->ToString() + "' found while scanning '" + rootDirectory.generic_string() + "'";
                    return false;
                }
    ...
    const AssetRecord* AssetRegistry::ResolveRecord(const AssetId& assetId) const
    {
        ...
        if (it == m_assetRecordsById.end()) { return nullptr; }

        return &it->second;
    }

    std::optional<AssetKind> AssetRegistry::ParseKind(const std::string_view text) { return AssetSchemaRegistry::ResolveBuiltinKind(text); }
