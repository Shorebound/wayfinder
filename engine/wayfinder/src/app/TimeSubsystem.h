#pragma once

#include "app/AppSubsystem.h"
#include "core/Result.h"
#include "wayfinder_exports.h"

#include <memory>

namespace Wayfinder
{
    class EngineContext;
    class Time;

    /**
     * @brief AppSubsystem wrapping the platform Time.
     *
     * Owns the Time instance and manages its lifecycle. Has no dependencies
     * on other subsystems and no required capabilities (always active).
     */
    class WAYFINDER_API TimeSubsystem : public AppSubsystem
    {
    public:
        TimeSubsystem() = default;
        ~TimeSubsystem() override;

        [[nodiscard]] auto Initialise(EngineContext& context) -> Result<void> override;
        void Shutdown() override;

        /// @pre Valid only after Initialise() and before Shutdown().
        [[nodiscard]] auto GetTime() -> Time&;

        /// @pre Valid only after Initialise() and before Shutdown().
        [[nodiscard]] auto GetDeltaTime() const -> float;

    private:
        std::unique_ptr<Time> m_time;
    };

} // namespace Wayfinder
