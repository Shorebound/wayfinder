...
namespace Wayfinder
{
    ...
    void* FrameAllocator::Allocate(size_t bytes, size_t alignment)
    {
        ...
        const auto base = reinterpret_cast<uintptr_t>(page.Memory.get());
        ...
        if (nextPage < m_pages.size() && needed <= m_pages[nextPage].Capacity)
        {
            ...
            const auto nextBase = reinterpret_cast<uintptr_t>(m_pages[nextPage].Memory.get());
        ...
        const auto newBase = reinterpret_cast<uintptr_t>(m_pages.back().Memory.get());
    ...
    {
        WAYFINDER_ASSERT(alignment != 0 && (alignment & (alignment - 1)) == 0, "AlignUp: alignment must be a non-zero power of two (got {})", alignment);
