...
namespace Wayfinder
{
    ...
    public:
        ...
        void Push(const TEvent& event) { m_active.push_back(event); }

        /** @brief Push an event by move into active storage. */
        void Push(TEvent&& event) { m_active.push_back(std::move(event)); }

        /** @brief Construct an event in-place in active storage. */
        template<typename... TArgs>
        TEvent& Emplace(TArgs&&... args) { return m_active.emplace_back(std::forward<TArgs>(args)...); }

        /** @brief Read-only span of events in active storage (pre-drain). */
        [[nodiscard]] std::span<const TEvent> Events() const { return m_active; }

        // ── IEventBuffer ────────────────────────────────────

        Event& At(size_t index) override { return m_draining[index]; }

        void PrepareForDrain() override { m_draining.swap(m_active); }

        void FinishDrain() override
        {
            if (m_active.capacity() < m_draining.capacity()) { m_active.reserve(m_draining.capacity()); }
            m_draining.clear();
        }
        ...
        [[nodiscard]] size_t Size() const override { return m_active.size(); }
