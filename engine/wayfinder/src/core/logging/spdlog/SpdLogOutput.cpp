...
namespace Wayfinder
{
    ...
    std::shared_ptr<ILogOutput> CreateConsoleOutput() { return std::make_shared<SpdConsoleOutput>(); }

    std::shared_ptr<ILogOutput> CreateFileOutput(const LogFileConfig& config) { return std::make_shared<SpdFileOutput>(config); }
    ...
    {
        if (m_sink) { m_sink->set_pattern(pattern); }
    }
