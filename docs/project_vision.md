...
Part of the fun is this opens up space to experiment with where rendering and engine tech *could* have gone if the industry hadn't spent twenty years optimising for photorealism. A lot of modern engines share the same assumptions because they're all solving the same problem. When you take realism off the table, some of those assumptions stop being obvious. Material models don't have to be metallic-roughness. Lighting doesn't have to conserve energy. Atmosphere doesn't have to simulate scattering. Post-processing doesn't have to mimic a camera lens, etc. So, hopefully there are genuine spaces to play in: artist-driven shading responses, mood-based fog systems, exaggerated physics tuned for feel rather than accuracy, colour palettes as a rendering primitive. It's a chance to ask "what if?" about the tech itself, not just the games.
