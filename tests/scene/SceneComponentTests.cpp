...
TEST_CASE("ValidateRenderOverride accepts empty table")
{
    Wayfinder::RuntimeComponentRegistry registry;
    registry.AddCoreEntries();
    ...
    toml::table table;
    std::string error;
    CHECK(registry.ValidateComponent("render_override", table, error));
    CHECK(error.empty());
}

TEST_CASE("ValidateRenderOverride accepts wireframe=true")
{
    Wayfinder::RuntimeComponentRegistry registry;
    registry.AddCoreEntries();

    toml::table table;
    table.insert_or_assign("wireframe", true);
    std::string error;
    CHECK(registry.ValidateComponent("render_override", table, error));
    CHECK(error.empty());
}

TEST_CASE("ValidateRenderOverride accepts wireframe=false")
{
    Wayfinder::RuntimeComponentRegistry registry;
    registry.AddCoreEntries();

    toml::table table;
    table.insert_or_assign("wireframe", false);
    std::string error;
    CHECK(registry.ValidateComponent("render_override", table, error));
    CHECK(error.empty());
}

TEST_CASE("ValidateRenderOverride rejects wireframe as string")
{
    Wayfinder::RuntimeComponentRegistry registry;
    registry.AddCoreEntries();

    toml::table table;
    table.insert_or_assign("wireframe", std::string("yes"));
    std::string error;
    CHECK_FALSE(registry.ValidateComponent("render_override", table, error));
    CHECK_FALSE(error.empty());
}

TEST_CASE("ValidateRenderOverride rejects wireframe as integer")
{
    Wayfinder::RuntimeComponentRegistry registry;
    registry.AddCoreEntries();

    toml::table table;
    table.insert_or_assign("wireframe", static_cast<int64_t>(1));
    std::string error;
    CHECK_FALSE(registry.ValidateComponent("render_override", table, error));
    CHECK_FALSE(error.empty());
}

// ── render_override component registry ───────────────────

TEST_CASE("render_override component is registered in the registry")
{
    Wayfinder::RuntimeComponentRegistry registry;
    registry.AddCoreEntries();

    CHECK(registry.IsRegistered("render_override"));
}

TEST_CASE("material component is registered in the registry")
{
    Wayfinder::RuntimeComponentRegistry registry;
    registry.AddCoreEntries();
...
// ── RenderOverrideComponent copy semantics ────────────────

TEST_CASE("RenderOverrideComponent copy preserves wireframe=true")
{
    Wayfinder::RenderOverrideComponent original;
    original.Wireframe = true;

    Wayfinder::RenderOverrideComponent copy = original;
    CHECK(copy.Wireframe.has_value());
    CHECK(*copy.Wireframe == true);
}

TEST_CASE("RenderOverrideComponent copy preserves wireframe=false")
{
    Wayfinder::RenderOverrideComponent original;
    original.Wireframe = false;

    Wayfinder::RenderOverrideComponent copy = original;
    CHECK(copy.Wireframe.has_value());
    CHECK(*copy.Wireframe == false);
}

TEST_CASE("RenderOverrideComponent copy preserves nullopt wireframe")
{
    Wayfinder::RenderOverrideComponent original;
    // Wireframe left as nullopt

    Wayfinder::RenderOverrideComponent copy = original;
    CHECK_FALSE(copy.Wireframe.has_value());
}

TEST_CASE("Modifying copy does not affect original RenderOverrideComponent")
{
    Wayfinder::RenderOverrideComponent original;
    original.Wireframe = true;
    ...
    CHECK(*original.Wireframe == true);
    CHECK(*copy.Wireframe == false);
}

// ── ApplyRenderOverride edge cases ────────────────────────

TEST_CASE("ApplyRenderOverride with no wireframe key leaves Wireframe as nullopt")
{
    flecs::world world;
    Wayfinder::Scene::RegisterCoreECS(world);
    Wayfinder::RuntimeComponentRegistry registry;
    registry.AddCoreEntries();
    registry.RegisterComponents(world);
    Wayfinder::Scene scene(world, registry, "ApplyRenderOverride Test");

    Wayfinder::Entity entity = scene.CreateEntity("TestEntity");

    // Apply render_override with no wireframe key
    toml::table componentTables;
    toml::table overrideTable;
    // intentionally no "wireframe" key
    componentTables.insert_or_assign("render_override", overrideTable);
...
// ── MaterialAsset parsing ignores wireframe ───────────────

TEST_CASE("ParseMaterialAssetDocument succeeds when wireframe key is present")
{
    toml::table doc;
    doc.insert_or_assign("asset_id", std::string("11111111-2222-3333-4444-555555555555"));
    doc.insert_or_assign("asset_type", std::string("material"));
    doc.insert_or_assign("name", std::string("TestMaterial"));
    doc.insert_or_assign("shader", std::string("unlit"));
    doc.insert_or_assign("wireframe", true); // legacy field — should be silently ignored

    Wayfinder::MaterialAsset asset;
    std::string error;
    bool ok = Wayfinder::ParseMaterialAssetDocument(doc, "test", asset, error);

    CHECK(ok);
    CHECK(error.empty());
    CHECK(asset.Name == "TestMaterial");
    CHECK(asset.ShaderName == "unlit");
}

TEST_CASE("CreateMaterialComponentTable does not write wireframe key")
{
    Wayfinder::MaterialAsset asset;
    asset.Id = *Wayfinder::AssetId::Parse("11111111-2222-3333-4444-555555555555");
    asset.Name = "TestMaterial";
    asset.ShaderName = "unlit";
    asset.SetBaseColor(Wayfinder::LinearColor{1.0f, 0.0f, 0.0f, 1.0f});

    toml::table table = Wayfinder::CreateMaterialComponentTable(asset);

    CHECK_FALSE(table.contains("wireframe"));
    CHECK(table.contains("material_id"));
    CHECK(table.contains("shader"));
}

TEST_CASE("ParseMaterialAssetDocument sets default white base_color when none provided")
{
    toml::table doc;
    doc.insert_or_assign("asset_id", std::string("11111111-2222-3333-4444-555555555555"));
    doc.insert_or_assign("asset_type", std::string("material"));
    ...
    CHECK(ok);
    CHECK(error.empty());
    // Default base_color should be white
    Wayfinder::LinearColor baseColor = asset.GetBaseColor();
    CHECK(baseColor.r == doctest::Approx(1.0f));
    CHECK(baseColor.g == doctest::Approx(1.0f));
    CHECK(baseColor.b == doctest::Approx(1.0f));
    CHECK(baseColor.a == doctest::Approx(1.0f));
}

// ── ValidateMaterial ignores wireframe key ────────────────

TEST_CASE("ValidateMaterial passes when wireframe key is absent")
{
    Wayfinder::RuntimeComponentRegistry registry;
    registry.AddCoreEntries();

    toml::table table;
    table.insert_or_assign("base_color", toml::array{255, 0, 0, 255});
    std::string error;
    CHECK(registry.ValidateComponent("material", table, error));
    CHECK(error.empty());
}